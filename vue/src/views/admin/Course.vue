      ...
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column label="操作" width="180" align="center">
          <template v-slot="scope">
            <el-button plain type="primary" @click="handleEdit(scope.row)" size="mini" v-if="user.role !== 'XUESHENG'">编辑</el-button>
    ...
    <el-dialog title="信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
        <el-form-item prop="name" label="课程名称">
          <el-input v-model="form.name" autocomplete="off" :disabled="user.role !== 'GUANLIYUAN'"></el-input>
        </el-form-item>
        <el-form-item prop="type" label="课程类型">
          <el-select v-model="form.type" placeholder="请选择类型" style="width: 100%" :disabled="user.role !== 'GUANLIYUAN'">
            <el-option label="必修" value="必修"></el-option>
            <el-option label="选修" value="选修"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="teacherId" label="授课教师">
          <el-select v-model="form.teacherId" placeholder="请选择教师" style="width: 100%" :disabled="user.role !== 'GUANLIYUAN'">
            <el-option v-for="item in teacherData" :label="item.name" :value="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="xuefen" label="学分">
          <el-input v-model="form.xuefen" autocomplete="off" :disabled="user.role !== 'GUANLIYUAN'"></el-input>
        </el-form-item>
        <el-form-item prop="renshu" label="上课人数">
          <el-input v-model="form.renshu" autocomplete="off" :disabled="user.role !== 'GUANLIYUAN'"></el-input>
        </el-form-item>
        <el-form-item prop="jiaoshi" label="上课教室">
          <el-input v-model="form.jiaoshi" autocomplete="off" :disabled="user.role !== 'GUANLIYUAN'"></el-input>
        </el-form-item>
        <el-form-item prop="xingqi" label="周几">
          <el-select v-model="form.xingqi" placeholder="请选择类型" style="width: 100%" :disabled="user.role !== 'GUANLIYUAN'">
        ...
        <el-form-item prop="segment" label="第几大节">
          <el-select v-model="form.segment" placeholder="请选择类型" style="width: 100%" :disabled="user.role !== 'GUANLIYUAN'">
            <el-option label="第一大节（08:00 ~ 10:20）" value="第一大节（08:00 ~ 10:20）"></el-option>
            <el-option label="第二大节（10:20 ~ 12:15）" value="第二大节（10:20 ~ 12:15）"></el-option>
            <el-option label="第三大节（13:30 ~ 15:20）" value="第三大节（13:30 ~ 15:20）"></el-option>
            <el-option label="第四大节（15:30 ~ 18:20）" value="第四大节（15:30 ~ 18:20）"></el-option>
            <el-option label="第五大节（18:20 ~ 21:20）" value="第五大节（18:20 ~ 21:20）"></el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    loadTeacher() {
      this.$request.get('/laoshi/selectAll').then(res => {
        if (res.code === '200') {
